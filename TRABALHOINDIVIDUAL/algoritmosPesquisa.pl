...
resolve_astar(Inicial, Final, CaminhoFinal/Custo) :-
	astarSearch([[Inicial]/0], Final, CaminhoReversed/Custo),
...
adjacenteAStar([Nodo|Caminho]/Custo, Final, [ProxNodo,Nodo|Caminho]/NovoCusto) :-
	        verificarLigacaoAStar( Nodo, ProxNodo),
...
melhorA(Inicial,Final,R):- findall((Ca,Cus), resolve_astar(Inicial,Final,Ca/Cus), L),minimo(L,R).
...
mais_rapidoA(Origem, Destino, Caminho/CostTime) :-
   resolve_astar(Origem,Destino,Caminho/CostTime),write("Caminho \n"),escrever(Caminho),displayCost(CostTime).
...
melhorMediaA(Inicial,Final,R):-
	findall((Ca,Cus), resolve_astar(Inicial,Final,Ca/Cus), L),minimoMedia(L,R).
