...
resolve_depthFirst(PontoRecolha, Inicial, Final, [(Inicial,R)|Caminho]) :-
	 procuraId(Inicial,PontoRecolha,L),eliminaRepetidos(L,R),
	 depthFirstSearch(PontoRecolha, Inicial, Final, Caminho),!.

depthFirstSearch(_, Final, Final, []) :- !.

depthFirstSearch(PontoRecolha, Nodo, Final, [(ProxNodo,R)|Caminho]) :-
    procuraId(ProxNodo,PontoRecolha,L),eliminaRepetidos(L,R),
    adjacenteDF(PontoRecolha, Nodo, ProxNodo),
    depthFirstSearch(PontoRecolha, ProxNodo, Final, Caminho).

adjacenteDF(_, Nodo, ProxNodo) :-
    ligacao(Nodo,ProxNodo,_).
   % existeNodo(Nodo, PontoRecolha),
   % existeNodo(ProxNodo, PontoRecolha).

existeNodo(Nodo, [pontoRecolha(_,_,_,_,Nodo,_,_,_)|_]).
existeNodo(Nodo, [_|Tail]) :- existeNodo(Nodo, Tail).
...
breadthFirstSearch(PontoRecolha, [[(N,Id)|Caminho]|CaminhoList], Final, Solucao) :-
	%procuraId(PN,PontoRecolha,L),eliminaRepetidos(L,R),
...
adjacenteBF(PontoRecolha, Nodo, (NodoAdj,Id)) :-
       ...
       %existeNodo(Nodo,PontoRecolha),
       %ligacao(NodoAdj,Nodo, _),
       %pontoRecolha(_,_,Id,_,NodoAdj,_,_,_).
       procuraId(NodoAdj,PontoRecolha,L),eliminaRepetidos(L,Id). % talvez lenght(L,R), R>0


% A* Search --------------------

resolve_astar(PontoRecolha, Inicial, Final, CaminhoFinal/Custo) :-
	astarSearch(PontoRecolha,[[(Inicial,'Inicio')]/0], Final, CaminhoReversed/Custo),
        inverteLista(CaminhoReversed, CaminhoFinal, []),!.

astarSearch(PontoRecolha, Caminhos, Final, Caminho) :-
	bestPathAsearch(Caminhos, Caminho),
	Caminho = [(Nodo,C)|_]/_, Nodo == Final.

astarSearch(PontoRecolha, Caminhos, Final, SolucaoCaminho) :-
	bestPathAsearch(Caminhos, MelhorCaminho),
	seleciona(MelhorCaminho, Caminhos, OutrosCaminhos),
	expand_astar(PontoRecolha, MelhorCaminho, Final, ExpCaminhos),
	append(OutrosCaminhos, ExpCaminhos, NovoCaminhos),
    astarSearch(PontoRecolha, NovoCaminhos, Final, SolucaoCaminho).
...
expand_astar(PontoRecolha, Caminho, Final, ExpCaminhos) :-
	findall(NovoCaminho, adjacenteAStar(PontoRecolha, Caminho, Final, NovoCaminho), ExpCaminhos).

adjacenteAStar(PontoRecolha, [(Nodo,C1)|Caminho]/Custo, Final, [(ProxNodo,C2),(Nodo,C1)|Caminho]/NovoCusto) :-
	        verificarLigacaoAStar(PontoRecolha, Nodo, (ProxNodo,C2)),
...
resolve_greedy(PontoRecolha, Inicial, Final, Caminho/Custo):-
	pontoRecolha(_,_,_,_,Inicial,_,_,Estima),
    greedySearch(PontoRecolha, [[(Inicial, 'Start')]/0/Estima], Final, InvCaminho/Custo/_),
	inverteLista(InvCaminho, Caminho, []),!.

greedySearch(PontoRecolha, Caminhos, Final, Caminho):-
        bestPathGreedy(Caminhos, Caminho),
        Caminho = [(Nodo,C)|_]/_/_, Nodo == Final.

greedySearch(PontoRecolha, Caminhos, Final, SolucaoCaminho):-
        bestPathGreedy(Caminhos, MelhorCaminho),
	seleciona(MelhorCaminho, Caminhos, OutrosCaminhos),
	expand_greedy(PontoRecolha, MelhorCaminho, Final, ExpCaminhos),
	append(OutrosCaminhos, ExpCaminhos, NovoCaminhos),
        greedySearch(PontoRecolha, NovoCaminhos, Final, SolucaoCaminho).
...
expand_greedy(PontoRecolha, Caminho, Final, ExpCaminhos):-
        findall(NovoCaminho, adjacenteAStar2(PontoRecolha, Caminho, Final, NovoCaminho), ExpCaminhos).

adjacenteAStar2(PontoRecolha, [(Nodo,C1)|Caminho]/Custo/_, Final, [(ProxNodo,C2),(Nodo,C1)|Caminho]/NovoCusto/Est) :-
		verificarLigacaoAStar2(PontoRecolha, Nodo, (ProxNodo,C2)),
			\+ membro((ProxNodo,C2), Caminho),
		ligacao( Nodo, ProxNodo, PassoCusto),
			NovoCusto is Custo + PassoCusto,
			pontoRecolha(_,_,_,_,ProxNodo,_,_,Est).
...
resolvePPLixo(Inicial,Final,[(Inicial,R)|Caminho],TipoLixo):-
    findall(Id,pontoRecolha(_,_,Id,_,Inicial,TipoLixo,_,_),R),
    primeiroprofundidadeCustoL(Inicial,Final,[(Inicial,R)],Caminho,TipoLixo).


primeiroprofundidadeCustoL(Nodo,Final, _, [],_):-Nodo ==Final.

primeiroprofundidadeCustoL(Nodo,Final,Historico,[(NodoProx,Id2)|Caminho],TipoLixo):-
   % findall(Id,pontoRecolha(_,_,Id,_,NodoProx,TipoLixo,_,_),Id2),
    adjacenteCustoL(Nodo,(NodoProx,Id2),TipoLixo),
    nao(member((NodoProx,Id2), Historico)),
    primeiroprofundidadeCustoL(NodoProx,Final,[(NodoProx,Id2)|Historico],Caminho,TipoLixo).

adjacenteCustoL(Nodo, (NodoProx,Id2),TipoLixo):-
    ligacao(Nodo,NodoProx,_),
    findall(Id,pontoRecolha(_,_,Id,_,NodoProx,TipoLixo,_,_),Id2).
   % pontoRecolha(_,_,Id,_,NodoProx,TipoLixo,_,_).
...
ppTodasSolucoesL(Inicial,Final,T,L):- findall(C,(resolvePPLixo(Inicial,Final,C,T)),R),eliminaRepetidos(R,L).

melhor(Inicial,Final,R):- findall((Ca,Cus), resolvePPCusto(Inicial,Final,Ca,Cus), L),minimo(L,R).

melhorMedia(Inicial,Final,R):- findall((Ca,Cus), resolvePPCusto(Inicial,Final,Ca,Cus), L),minimoMedia(L,R).

% 1-----------------------------------------------------------------------------------------------------------------------

calcula_traj_tipoLixoDF(Origem, Destino, TipoLixo,Caminho/0) :-
      %  findall(pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), PontosRecolha),
      %  resolve_depthFirst(PontosRecolha, Origem, Destino, Caminho),write("Caminho \n"),escrever(Caminho).
	  ppTodasSolucoesL(Origem,Destino,TipoLixo,Caminho),write("Caminho \n"),escrever(Caminho).

calcula_traj_tipoLixoBF(Origem, Destino, TipoLixo,Caminho/0) :-
	findall(pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), PontosRecolha),
	resolve_breathFirst(PontosRecolha, Origem, Destino, Caminho),write("Caminho \n"),escrever(Caminho).


calcula_traj_tipoLixoA(Origem, Destino, TipoLixo,Caminho/CostTime) :-
	%findall(pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), PontosRecolha),
	resolve_aestrela(Origem, Destino, Caminho/CostTime,TipoLixo),write("Caminho \n"),escrever(Caminho),displayCost(CostTime).

calcula_traj_tipoLixoG(Origem, Destino, TipoLixo,Caminho/CostTime) :-
	%findall(pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), PontosRecolha),
	resolveGulosa(Origem, Destino, Caminho/CostTime,TipoLixo),write("Caminho \n"),escrever(Caminho),displayCost(CostTime).
...
mais_rapidoA(Origem, Destino, Caminho/CostTime) :-
	findall(pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), PontosRecolha),
    resolve_astar(PontosRecolha,Origem, Destino, Caminho/CostTime),write("Caminho \n"),escrever(Caminho),displayCost(CostTime).

mais_rapidoG(Origem, Destino, Caminho/CostTime) :-
	findall(pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), pontoRecolha(La,Lo,Id,F,Nodo,TipoLixo,C,Cap), PontosRecolha),
    resolve_greedy(PontosRecolha,Origem ,Destino, Caminho/CostTime),write("Caminho \n"),escrever(Caminho),displayCost(CostTime).

mais_rapidoP(Origem, Destino) :-
	melhor(Origem ,Destino,Caminho),
	write("Caminho\n"),displayList(Caminho).
...
%------------------------------------------------------------------------------
resolve_aestrela(Nodo,Final, Caminho/Custo,TipoLixo) :-
	findall(Id,pontoRecolha(_,_,Id,_,Nodo,TipoLixo,_,_),R),
	aestrela([[(Nodo,R)]/0], Final,InvCaminho/Custo,TipoLixo),
	inverso(InvCaminho, Caminho).

aestrela(Caminhos,Final, Caminho,_) :-
	obtem_melhor(Caminhos, Caminho),
	Caminho = [(Nodo,_)|_]/_,Nodo==Final.

aestrela(Caminhos,Final, SolucaoCaminho,TipoLixo) :-
	obtem_melhor(Caminhos, MelhorCaminho),
	seleciona(MelhorCaminho, Caminhos, OutrosCaminhos),
	expande_aestrela(MelhorCaminho, ExpCaminhos,TipoLixo),
	append(OutrosCaminhos, ExpCaminhos, NovoCaminhos),
    aestrela(NovoCaminhos,Final, SolucaoCaminho,TipoLixo).
...
expande_aestrela(Caminho, ExpCaminhos,TipoLixo) :-
	findall(NovoCaminho, adjacente(Caminho,NovoCaminho,TipoLixo), ExpCaminhos).

adjacente([(Nodo,Id)|Caminho]/Custo, [(ProxNodo,R),(Nodo,Id)|Caminho]/NovoCusto,TipoLixo) :-
	ligacao(Nodo, ProxNodo, PassoCusto),\+ member(ProxNodo, Caminho),
	findall(ID,pontoRecolha(_,_,ID,_,ProxNodo,TipoLixo,_,_),R),
	NovoCusto is Custo + PassoCusto.

aTodasSolucoesL(Inicial,Final,T,L):- findall(C,(resolve_aestrela(Inicial,Final,C/Cu,T)),R),eliminaRepetidos(R,L).


resolveGulosa(Nodo, Final,Caminho/Custo, TipoLixo) :-
    findall(Id,pontoRecolha(_,_,Id,_,Nodo,TipoLixo,_,_),R),
    agulosa([[(Nodo,R)]/0],Final, CaminhoInverso/Custo,TipoLixo),
    inverso(CaminhoInverso, Caminho).

agulosa(Caminhos,Final, Caminho,_) :-
    obtem_melhor_g(Caminhos, Caminho),
    Caminho = [(Nodo,_)|_]/_,Nodo==Final.

agulosa(Caminhos,Final, SolucaoCaminho,TipoLixo) :-
    obtem_melhor_g(Caminhos, MelhorCaminho),
    seleciona(MelhorCaminho, Caminhos, OutrosCaminhos),
    expandeGulosa(MelhorCaminho, ExpCaminhos,TipoLixo),
    append(OutrosCaminhos, ExpCaminhos, NovoCaminhos),
    agulosa(NovoCaminhos,Final, SolucaoCaminho,TipoLixo).
...
expandeGulosa(Caminho, ExpCaminhos,TipoLixo) :-
	findall(NovoCaminho, adjacenteG(Caminho,NovoCaminho,TipoLixo), ExpCaminhos).

adjacenteG([(Nodo,Id)|Caminho]/Custo, [(ProxNodo,R),(Nodo,Id)|Caminho]/NovoCusto,TipoLixo) :-
	ligacao(Nodo, ProxNodo, PassoCusto),\+ member(ProxNodo, Caminho),
	findall(ID,pontoRecolha(_,_,ID,_,ProxNodo,TipoLixo,_,_),R),
	NovoCusto is Custo + PassoCusto.

gTodasSolucoesL(Inicial,Final,T,L):- findall(C,(resolveGulosa(Inicial,Final,C/Cu,T)),R),eliminaRepetidos(R,L).

g1TodasSolucoesL(Inicial,Final,T,L):- findall(C,(calcula_traj_tipoLixoG(Inicial,Final,C/Cu,T)),R),eliminaRepetidos(R,L).

%5------------------------------------------------------------------------------------------------

mais_rapidoMediaP(Origem, Destino) :-
